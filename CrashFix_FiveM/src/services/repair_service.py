...
FIX: repair_functions mapping corregido para coincidir con los valores del modal HTML.
     validate_repair_ids ya no se llama dos veces (se llama solo en app.py).
"""
...
class RepairService:
    ...
    def clear_fivem_cache_selective(self) -> Dict[str, Any]:
        ...
        if not os.path.exists(cache_path):
            return {'success': False, 'error': 'Carpeta de caché no encontrada', 'cleaned_mb': 0}
        ...
        return {'success': True, 'cleaned_mb': size_mb, 'cleared': cleared,
    ...
    def remove_conflicting_dlls(self) -> Dict[str, Any]:
        ...
        return {'success': True, 'found': found, 'removed': removed,
                'errors': errors if errors else None}
    ...
    def repair_ros_authentication(self) -> Dict[str, Any]:
        ...
        for filepath in files_to_delete:
            if os.path.exists(filepath):
                try:
                    backup_item(filepath, os.path.basename(filepath),
        ...
        return {'success': True, 'deleted': deleted, 'errors': errors if errors else None}
    ...
    def disable_gta_mods(self, gta_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {'success': True, 'disabled_count': disabled,
                'errors': errors if errors else None}
    ...
    def close_conflicting_software(self) -> Dict[str, Any]:
        ...
        for conflict in conflicts.get('ConflictsFound', []):
            if conflict in process_map:
                result = run_command(['taskkill', '/F', '/IM', process_map[conflict]], timeout=10)
    ...
    def add_firewall_exclusions(self) -> Dict[str, Any]:
        ...
        try:
            run_command(['netsh', 'advfirewall', 'firewall', 'add', 'rule',
                         ...
                         f'program={fivem_exe}', 'enable=yes'], timeout=10)
            run_command(['netsh', 'advfirewall', 'firewall', 'add', 'rule',
    ...
    def add_defender_exclusions(self) -> Dict[str, Any]:
        ...
        for path in paths_to_exclude:
            if os.path.exists(path):
                result = run_powershell(f'Add-MpPreference -ExclusionPath "{path}"', timeout=10)
    ...
    def run_advanced_repair(self, selected_repairs: List[int]) -> Dict[str, Any]:
        """
        FIX: Mapeo corregido para que coincida con los valores del modal HTML del frontend.
        El frontend manda valores del 1 al 13 con este significado:
          1  Terminar procesos de FiveM
          2  Limpiar caché selectiva
          3  Limpiar caché completa
          4  Eliminar DLLs conflictivas
          5  Limpiar v8 DLLs (system32)  ← mismo método que 4
          6  Limpiar archivos ROS
          7  Reparar autenticación ROS
          8  Desactivar mods de GTA V
          9  Cerrar software conflictivo
         10  Configurar reglas de Firewall
         11  Optimizar configuración gráfica
        ...
        repair_functions: Dict[int, tuple] = {
            ...
            5:  ('Limpiar v8 DLLs (system32)', self.remove_conflicting_dlls),
            6:  ('Limpiar archivos ROS',       self.repair_ros_authentication),
            7:  ('Reparar autenticación ROS',  self.repair_ros_authentication),
            ...
            13: ('Optimizaciones de Windows',  self._optimize_windows),
        }
        ...
        for repair_id in selected_repairs:
            if repair_id in repair_functions:
                ...
                try:
                    ...
                    results.append({'id': repair_id, 'name': name, 'success': success, 'details': result})
                except Exception as e:
                    logger.error(f"Error en reparación {name}: {e}")
                    results.append({'id': repair_id, 'name': name, 'success': False, 'error': str(e)})
...
<Settings>
    ...
    <DOF value="0"/>
        ...
        try:
            backup_item(settings_path, 'settings.xml', self.paths.backup_folder, 'Config')
    ...
    def _configure_texture_budget(self) -> Dict[str, Any]:
        ...
        return {'success': True, 'vram_detected': vram, 'recommended_budget': budget}
